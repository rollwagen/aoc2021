...
def part_one_and_two() -> int:

    binary_most_common, binary_least_common = _calculate_most_and_least_common_bits(input)
    ...
    def _filter_rating(index: int, oxy_gen_list, bit_criteria_most=True) -> list:
        ...
        oxy_most_common, oxy_least_common = _calculate_most_and_least_common_bits(oxy_gen_list)
        # print(f'{oxy_most_common=}')
        if bit_criteria_most is True:
            new_list = [item for item in oxy_gen_list if int(item[index]) == oxy_most_common[index]]
        else:
            new_list = [item for item in oxy_gen_list if int(item[index]) == oxy_least_common[index]]
        # print(f'{new_list=}')
        return new_list
    ...
    print(f"part_two = {int(oxygen_generator[0], 2) * int(co2_scrubber[0], 2)}  (1370737)")
